...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 21 13:36:48 2020
...
real_faults_coords_saved =      True #
#### !! Don't change these for UB-18 

n_subsampled_wells = 5
...
run = run_list[nmbr]
obs_optn = obs_optn_list[nmbr] # DON'T CHANGE
ph_optn = ph_optn_list[nmbr]   # DON'T CHANGE
...
for run, obs_optn, ph_optn in zip(run_list, obs_optn_list, ph_optn_list):
    ...
    for f in range(n_ph_fault_setups):
            ...
            if ph_fa_length_list[i] == 0:
                ...
                while ph_fa_length_list[i] == 0:
                    fault_coordinate_redone = functions_proj_3.barriers.makeFaultCoords(1, 
                                                            ...
                                                            param_dict["ncol"],
                                                            param_dict["delr"])    
                    ph_f_coords[f][i] = fault_coordinate_redone[0]
                    ph_fa_rows_list, ph_fa_cols_list, ph_fa_length_list = (
                            functions_proj_3.barriers.makeFaultRowCols(ph_f_coords[f], 
                                                                       param_dict["nrow"], 
                                                                       param_dict["ncol"]))
                    ph_fa_length_list[0]
    ...
    for x in range(n_ph_fault_setups):
        for barr_number in range(n_phantom_faults):
            for i in range(ph_f_lengths[x][barr_number]):
                hk_ph[0, int(ph_f_rows[x][barr_number][i]), 
                int(ph_f_cols[x][barr_number][i])] = 1e-3  
    ...
    for x in range(n_ph_fault_setups):
        for barr_number in range(n_phantom_faults):
            for i in range(ph_f_lengths[x][barr_number]):
                hk_ph[0, int(ph_f_rows[x][barr_number][i]), 
                int(ph_f_cols[x][barr_number][i])] = 0        
    ...
    if real_faults_coords_saved == False:
        re_fault_coordinates = functions_proj_3.barriers.makeFaultCoords(1, 
                                                               param_dict["nrow"], 
                                                               param_dict["ncol"],
                                                               param_dict["delr"])
                                                               
               
        np.save(os.path.join(overarching_model_folder, "re_fault_coords"), re_fault_coordinates)
    else:
        re_fault_coordinates = np.load(os.path.join('C:\\workspace\\Proj3_PilbaraBarriers\\UB_54\\re_fault_coords'))
...
#    plt.legend(loc=4) # legend_list  
    ...
    for barr_number in range(n_phantom_faults):
        for row, col in zip(ph_f_rows[ph_optn][barr_number], ph_f_cols[ph_optn][barr_number]):
    ...
    ph_reg_df = pd.DataFrame({
                              ...
                              "obsval": prior_vals_pf,
                              "obsnme": obs_name_pf_reg,
                              "ins_line": ins_line_pf,
                              "weight": weight_pf_reg,
                              "obsgnme": obsgnme_pf_reg})
    
    fileName_text = os.path.join(this_model_folder, "regul_ehk_obs.out.ins")
    ph_reg_df.to_csv(fileName_text, columns=["ins_line"], header=True, index=False)
    ph_reg_df.index = ph_reg_df.obsnme
    # Now I need to change the header "ins_line" to "pif~"
    ...
    # Check if my naming convention worked, i.e. if there are duplicates.
    
    a = ph_reg_df['obsnme'].duplicated()
    len(a[a==True])
    len(ph_reg_df['obsnme'])
    #--------------------------------------------------------------------------
    ...
    if phantom_or_pilot == "Phantom_faults":
        ...
        '''
        Why are my pilot points not systematic across the domain? They start 6 cells in, not symmetric... 
        '''
    ...
    if phantom_or_pilot == "Phantom_faults":
        ...
        # Save the pilot point dataframe
        fileName_pp = os.path.join(overarching_model_folder, "pilotpoints.csv")
        hk_pp.to_csv(fileName_pp, encoding='utf-8', index=False)
        ...
        # PLAYING AROUND WITH PILOT POINT GRID
        x_pps = np.linspace(550,18550, 19) # what i get
        x_pps2 = np.linspace(1000,19000, 19) # what i want
        y_pps = np.linspace(1450, 9450, 9)
        y_pps2 = np.linspace(1000, 9000, 9)        
        
        # Plot the kriging variance array
        fig = plt.figure(figsize=(10,10))
        ax = plt.subplot(111,aspect="equal")
        ax.pcolormesh(ml.sr.xcentergrid,ml.sr.ycentergrid,var_arr,alpha=0.5)
        ax.scatter(hk_pp.x[1], hk_pp.y[1],marker='.',s=10, c='g')
...
#    output_files = [f.replace(".ins",'') for f in ins_files]
    ...
    pst.observation_data.loc[hob_and_age_df.index, "obsval"] = hob_and_age_df.obsval
     ...
     # Change the details of the regularisation data --------------------------
    pst.observation_data.loc[ph_reg_df.index,"obgnme"] = ph_reg_df.obsgnme
    pst.observation_data.loc[ph_reg_df.index,"weight"] = ph_reg_df.weight
    pst.observation_data.loc[ph_reg_df.index, "obsval"] = ph_reg_df.obsval
    ...
    if phantom_or_pilot == "Phantom_faults":
        ...
        if include_real_with_ph == True:
            print("Need to copy and make sure forward run file is correct - haven't set this up")
